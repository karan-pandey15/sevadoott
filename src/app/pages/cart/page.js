...
const CartPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      <div className="max-w-4xl mx-auto w-full bg-white min-h-screen shadow-sm relative">
        {/* ── Sticky Header ────────────────────────────────────────── */}
        ...
        <main className="w-full px-4 md:px-8 pt-6 pb-40">
          ...
          {items.length === 0 ? (
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-7 space-y-4">
                  {items.map((item, idx) => (
                    <motion.div 
                      ...
                      className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 flex gap-4 relative overflow-hidden group hover:shadow-md transition-shadow"
                    >
                      {/* Item Image */}
                      <div className="w-24 h-24 md:w-32 md:h-32 bg-gray-50 rounded-2xl overflow-hidden flex-shrink-0 border border-gray-50 flex items-center justify-center text-4xl relative">
                        {item.emoji || "✨"}
                        {item.image && (
                          <img 
                      ...
                      <div className="flex-1 min-w-0 flex flex-col justify-between">
                        <div>
                          <h3 className="font-black text-gray-800 text-base md:text-lg leading-tight truncate uppercase italic">
                            {item.name}
                          </h3>
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <p className="text-lg md:text-xl font-black text-[#106670]">₹{item.price * item.quantity}</p>
                ...
                {/* Bill Summary & Policies */}
                <div className="lg:col-span-5 space-y-6">
                  {/* Bill Summary Card */}
                  <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 space-y-4 lg:sticky lg:top-24">
        ...
        <AnimatePresence>
          {items.length > 0 && (
            <motion.div 
              ...
              animate={{ y: 0 }}
              exit={{ y: 100 }}
              className="sticky bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-6 z-[100] rounded-t-[40px] shadow-[0_-20px_40px_rgba(0,0,0,0.05)]"
            >
              <div className="flex items-center justify-between gap-4">
                ...
                <button 
                  ...
                  className="flex-1 max-w-xs bg-[#1898A5] text-white py-4 rounded-2xl font-black text-lg shadow-xl shadow-blue-200 active:scale-95 transition-all flex items-center justify-center gap-2 group italic"
                >
                  PROCEED TO PAY
